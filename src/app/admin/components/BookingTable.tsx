...
export default function BookingTable() {
  ...
  const confirmMutation = api.booking.confirm.useMutation({
    ...
    onError: (error) => {
      window.alert(`Confirmation failed: ${error.message}`);
    },
  });
  ...
  const rejectMutation = api.booking.reject.useMutation({
    ...
    onError: (error) => {
      window.alert(`Rejection failed: ${error.message}`);
    },
  });

  const bookings = useMemo<Booking[]>(() => {
    return (bookingsQuery.data ?? []) as Booking[];
  }, [bookingsQuery.data]);
  ...
  return (
    <div className="min-h-screen bg-[#050706] px-4 py-6 text-[#f7efe0] sm:px-6 lg:px-8">
                      ...
                      {filteredBookings.length === 0 ? (
                        ...
                        filteredBookings.map((booking) => (
                          ...
                          >
                            ...
                            <td className="px-5 py-4">
                              <span
                                className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-bold ${statusStyles[booking.status]}`}
                              >
          ...
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-[#0b100d] shadow-2xl">
            ...
            <div className="space-y-5 p-5">
              ...
              <div className="flex items-center justify-between border-t border-white/10 pt-5">
                ...
                <span
                  className={`rounded-full border px-3 py-1.5 text-[10px] font-bold ${statusStyles[selectedBooking.status]}`}
                >
            ...
            {selectedBooking.status === "COMPLETED" && (
              <div className="border-t border-white/10 p-5">
                <div className="rounded-xl border border-blue-400/20 bg-blue-400/5 p-4 text-center">
                  <CalendarCheck className="mx-auto h-6 w-6 text-blue-300" />
