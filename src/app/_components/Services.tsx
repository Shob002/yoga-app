...
export default function Services() {
  return (
    <section className="relative overflow-hidden bg-[#050706] px-6 py-24 text-[#f7efe0]">
      {/* Background glow */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,#2d6b4b22,transparent_40%)]" />
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        >
          ...
          <h2 className="mt-5 max-w-5xl text-[clamp(2.5rem,5vw,5rem)] font-black leading-[0.95] tracking-tighter">
            Complete Wellness{" "}
            <span className="text-[#7bae8a]">Ecosystem</span>
          </h2>
          ...
          {services.map((service, index) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -10 }}
                className="group rounded-[2.5rem] border border-white/10 bg-white/[0.035] p-8 backdrop-blur transition hover:border-[#d6b36a]/40 hover:shadow-[0_0_70px_#d6b36a18]"
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-[#7bae8a]/10 text-[#7bae8a] transition group-hover:bg-[#d6b36a] group-hover:text-[#050706]">
                    <Icon className="h-8 w-8" />
                  </div>

                  <span className="rounded-full border border-[#d6b36a]/20 px-3 py-1 text-[10px] font-black uppercase tracking-widest text-[#d6b36a]">
                    {service.tag}
                  </span>
                </div>
                ...
                <Link
                  href="/booking"
                  className="mt-7 inline-flex items-center gap-2 text-sm font-black uppercase tracking-widest text-[#d6b36a] transition group-hover:gap-4"
                >
                  Explore
        ...
        >
          ...
          <Link
            href="/booking"
            className="mt-7 inline-flex rounded-full bg-[#d6b36a] px-8 py-4 text-sm font-black uppercase tracking-widest text-[#050706] transition hover:bg-[#f2d89a]"
          >
            Book Assessment
          </Link>
        </motion.div>
